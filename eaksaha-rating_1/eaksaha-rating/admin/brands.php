...
<h2 class="section-title"><?= $editData ? 'แก้ไขแบรนด์' : 'เพิ่มแบรนด์ใหม่' ?></h2>
<div class="form-card">
    <form method="POST">
        ...
        <div class="field">
            ...
            <input type="color" id="color" name="color" value="<?= htmlspecialchars($editData['color'] ?? '#E10600') ?>" style="height:46px; padding:5px;">
...
<h2 class="section-title">รายชื่อแบรนด์ทั้งหมด</h2>
            ...
            <?php else: ?>
                <?php foreach ($brands as $b): ?>
                    <tr>
                        ...
                        <td><span class="brand-tag" style="background:<?= htmlspecialchars($b['color']) ?>"><?= htmlspecialchars($b['name']) ?></span></td>
                        <td><code><?= htmlspecialchars($b['code']) ?></code></td>
                        <td><span style="display:inline-block;width:22px;height:22px;border-radius:6px;border:1px solid #2A2A2E;vertical-align:middle;background:<?= htmlspecialchars($b['color']) ?>"></span> <?= htmlspecialchars($b['color']) ?></td>
                        <td><?= (int) $b['staff_count'] ?> คน</td>
                        <td>
                            <a href="brands.php?edit=<?= (int) $b['id'] ?>" class="btn btn-secondary btn-sm">แก้ไข</a>
