...
function rpIcon(string $name): string
{
    ...
    switch ($name) {
        ...
        case 'comment':  return $open . '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>';
...
function reportFilterBar(
    ...
    ?int $brandId, ?int $staffId, bool $hasFeedback,
...
<div class="filter-bar" style="flex-direction:column; align-items:stretch; gap:14px;">
    ...
    <form class="filter-group" method="GET" data-ajax data-ajax-auto>
        ...
        <label class="check-pill <?= $hasFeedback ? 'active' : '' ?>">
            <input type="checkbox" name="has_feedback" value="1" <?= $hasFeedback ? 'checked' : '' ?>>
            <?= rpIcon('comment') ?>เฉพาะที่มีข้อเสนอแนะ
        </label>
...
<table>
    ...
    <tbody>
        ...
        <?php else: ?>
            <?php foreach ($ratings as $row): $sc = (int) $row['score']; ?>
                <tr>
                    ...
                    <td class="stars-display" aria-label="<?= $sc ?> จาก 5 คะแนน"><span aria-hidden="true"><?= str_repeat('★', $sc) . str_repeat('☆', 5 - $sc) ?></span></td>
