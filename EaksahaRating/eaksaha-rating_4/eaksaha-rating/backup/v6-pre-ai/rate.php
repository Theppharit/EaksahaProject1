...
// ══════════════════════════════════════════════════════════
//  บันทึกผลประเมิน
//  หมายเหตุ: ส่วนเลือกคะแนน 1-5 ถูกถอดออกจากหน้าจอแล้ว
//  ตรรกะบันทึกยังอยู่ครบ เพื่อให้เสียบวิธีให้คะแนนแบบใหม่เข้ามาได้ทันที
//  โดยส่งค่า score (1-5) มากับฟอร์มเหมือนเดิม
// ══════════════════════════════════════════════════════════
...
<body>
            ...
            <?php else: ?>
                ...
                <div class="card-body">
                    ...
                    <form method="POST" id="rateForm">
                        ...
                        <!-- ช่องรับคะแนน — วิธีให้คะแนนแบบใหม่จะมาเติมค่าตรงนี้ -->
                        <input type="hidden" name="score" id="scoreInput" value="">

                        <div class="feedback-field">
                            <label for="feedback">ข้อเสนอแนะเพิ่มเติม (ถ้ามี)</label>
                            <textarea name="feedback" id="feedback" rows="4" maxlength="1000"
                                placeholder="เช่น ให้ข้อมูลรถละเอียด แนะนำโปรโมชั่นดี บริการสุภาพเป็นกันเอง..."></textarea>
                            <div class="char-row">
                                <span class="char-hint">ไม่บังคับกรอก</span>
                                <span class="char-count" id="charCount" aria-live="polite">0 / 1000</span>
                            </div>
                        </div>

                        <!-- ที่ว่างสำหรับวิธีให้คะแนนแบบใหม่ -->
                        <div class="score-slot">
                            <div class="slot-ic" aria-hidden="true">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4z"/>
                                </svg>
                            </div>
                            <div class="slot-title">ส่วนให้คะแนนกำลังปรับปรุง</div>
                            <div class="slot-note">รูปแบบการให้คะแนนใหม่จะมาแทนที่ตรงนี้เร็วๆ นี้</div>
                        </div>
                    </form>
                </div>
    ...
    (function () {
        ...
        // ── ฟังก์ชันส่งคะแนน (พร้อมใช้ทันทีเมื่อเพิ่มวิธีให้คะแนนแบบใหม่) ──
        // เรียก submitScore(1..5) จากปุ่มหรือคอนโทรลแบบใหม่ได้เลย
        window.submitScore = function (score) {
            score = parseInt(score, 10);
            if (!(score >= 1 && score <= 5)) return;
            var form = document.getElementById('rateForm');
            if (!form || form.dataset.sent === '1') return;   // กันกดซ้ำ
            form.dataset.sent = '1';
