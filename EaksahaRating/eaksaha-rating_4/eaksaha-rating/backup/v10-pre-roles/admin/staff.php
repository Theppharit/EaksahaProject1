...
if (defined('PUBLIC_BASE_URL') && PUBLIC_BASE_URL !== '') {
    // ใช้ที่อยู่ที่ตั้งไว้เองใน conn/config.php (แนะนำสำหรับใช้งานจริง)
    $rateBaseUrl = rtrim(PUBLIC_BASE_URL, '/') . '/rate.php?code=';
} else {
    // เดาจากที่อยู่ที่กำลังเปิดอยู่
    $protocol  = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host      = $_SERVER['HTTP_HOST'];
    $scriptDir = dirname(dirname($_SERVER['SCRIPT_NAME'])); // ขึ้นหนึ่งระดับจาก /admin/
    // เข้ารหัสทีละส่วนของ path — ถ้าชื่อโฟลเดอร์มีช่องว่างหรือวงเล็บ
    // แล้วไม่เข้ารหัส QR จะสแกนไม่ติดหรือถูกตัดกลางทาง
    $segs      = array_map('rawurlencode', array_filter(explode('/', $scriptDir), 'strlen'));
    $safeDir   = $segs ? '/' . implode('/', $segs) : '';
    $rateBaseUrl = $protocol . '://' . $host . $safeDir . '/rate.php?code=';
}

// เตือนถ้าลิงก์ยังชี้มาที่เครื่องตัวเอง — มือถือลูกค้าจะเปิดไม่ได้
$qrHost      = parse_url($rateBaseUrl, PHP_URL_HOST) ?: '';
$qrIsLocal   = in_array(strtolower($qrHost), ['localhost', '127.0.0.1', '::1'], true);
...
<?php if ($qrIsLocal): ?>
<div class="alert alert-error" style="display:flex;gap:11px;align-items:flex-start;">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:20px;height:20px;flex-shrink:0;margin-top:1px;">
        <path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
    </svg>
    <div>
        <b>QR ที่สร้างตอนนี้ใช้กับมือถือลูกค้าไม่ได้</b><br>
        ลิงก์ชี้ไปที่ <code><?= htmlspecialchars($qrHost) ?></code> ซึ่งหมายถึง “เครื่องนี้” เท่านั้น
        เมื่อลูกค้าสแกนด้วยมือถือจะเปิดไม่ขึ้น<br>
        วิธีแก้: เปิดไฟล์ <code>conn/config.php</code> แล้วตั้งค่า
        <code>PUBLIC_BASE_URL</code> เป็นที่อยู่จริงที่เครื่องอื่นเข้าถึงได้
        เช่น <code>http://192.168.1.50/eaksaha-rating</code> (IP ของเครื่องนี้ในวง LAN)
        หรือชื่อโดเมนจริงเมื่อขึ้นเซิร์ฟเวอร์
    </div>
</div>
<?php endif; ?>
                ...
                <?php foreach ($staffList as $row): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="#" id="dl-<?= (int)$row['id'] ?>" class="btn btn-secondary btn-sm" style="margin-top:6px;"
...
(function () {
    ...
    window.downloadQR = function (id, name) {
        ...
        const tryExport = function () {
            ...
            if (canvas) { try { finish(withQuietZone(canvas)); } catch (e) { if (btn) { btn.textContent = 'โหลดไม่สำเร็จ ลองใหม่'; btn.style.pointerEvents = 'auto'; } } return true; }
