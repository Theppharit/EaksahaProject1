...
$message = '';
$messageType = '';
$roleNamesFlash = ['admin' => 'ผู้ดูแลระบบ', 'manager' => 'ผู้จัดการ', 'sales' => 'พนักงานขาย'];
    ...
    try {
        ...
        if ($action === 'create' || $action === 'update') {
            ...
            if ($action === 'create') {
                ...
                flash('success', 'เพิ่มบัญชี ' . $username . ' แล้ว',
                      'สิทธิ์: ' . ($roleNamesFlash[$role] ?? $role) . ' — ล็อกอินได้ทันทีด้วยรหัสผ่านที่ตั้งไว้',
                      ['text' => 'ดูในบัญชีทั้งหมด', 'href' => '#row-' . $newId]);
                flash_redirect('users.php?hl=' . $newId);
            } else {
                if ($password !== '') {
                    if (strlen($password) < 8) {
                        throw new RuntimeException('รหัสผ่านต้องยาวอย่างน้อย 8 ตัวอักษร');
                ...
                flash('success', 'บันทึกการแก้ไขแล้ว',
                      $username . ' · สิทธิ์: ' . ($roleNamesFlash[$role] ?? $role),
                      ['text' => 'ดูในบัญชีทั้งหมด', 'href' => '#row-' . $id]);
                flash_redirect('users.php?hl=' . $id);
            }
            $messageType = 'success';
        }
        ...
        if ($action === 'toggle') {
            ...
            flash('success',
                  ...
                  $newActive === 1 ? 'บัญชีนี้ล็อกอินเข้าระบบได้แล้ว' : 'บัญชีนี้จะล็อกอินไม่ได้ แต่ประวัติข้อความที่เคยฝากไว้ยังอยู่ครบ',
                  ['text' => 'ดูในบัญชีทั้งหมด', 'href' => '#row-' . $id]);
            flash_redirect('users.php?hl=' . $id);
        }

        // ---------- ลบบัญชีถาวร ----------
        // เจ้าของงานสั่งให้มีปุ่มนี้เอง (2026-08-08)
        //
        // ทำไมถึงลบได้โดยไม่เสียประวัติ:
        // ตาราง review_notes เก็บ author_name / author_role ไว้เป็นสำเนาตั้งแต่ตอนส่งข้อความ
        // ไม่ได้ join เอาชื่อจาก admin_users ตอนแสดงผล ข้อความสั่งงานจึงยังอ่านได้ครบ
        // ว่าใครเป็นคนพูด แม้บัญชีนั้นจะไม่มีอยู่แล้ว (FK ตั้ง ON DELETE SET NULL ไว้)
        //
        // ด่านกันพลาดเหมือนกับตอนปิดใช้งานทุกประการ — ลบเป็นการกระทำที่หนักกว่า
        // จึงต้องไม่หลวมกว่ากัน
        if ($action === 'delete') {
            if ($id === $meId) {
                throw new RuntimeException('ลบบัญชีตัวเองไม่ได้ ให้ผู้ดูแลคนอื่นเป็นคนลบให้');
            }
            ...
            // ถ้าลบคนนี้แล้วไม่เหลือ admin ที่ใช้งานได้เลย จะไม่มีใครเข้าหลังบ้านได้อีก
            if ($u['role'] === 'admin' && activeAdminCount($pdo, $id) === 0) {
                throw new RuntimeException('ต้องมีผู้ดูแลระบบที่ใช้งานได้อย่างน้อย 1 บัญชี — สร้างบัญชีผู้ดูแลอีกคนก่อนแล้วค่อยลบบัญชีนี้');
            }

            // นับข้อความที่บัญชีนี้เคยฝากไว้ เพื่อบอกในป๊อปอัปว่าไม่ได้หายไปไหน
            $noteCount = 0;
            try {
                $ns = $pdo->prepare('SELECT COUNT(*) FROM review_notes WHERE author_id = ?');
                $ns->execute([$id]);
                $noteCount = (int) $ns->fetchColumn();
            } catch (PDOException $e) {
                // ยังไม่ได้รัน roles_migration — ไม่ใช่เหตุให้ลบไม่ได้
            }
            ...
            flash('success', 'ลบบัญชี ' . $u['username'] . ' แล้ว',
                  $noteCount > 0
                      ? 'ข้อความที่บัญชีนี้เคยฝากถึงพนักงาน ' . number_format($noteCount) . ' รายการยังอยู่ครบ พร้อมชื่อผู้ส่งเดิม'
                      : 'บัญชีนี้ล็อกอินเข้าระบบไม่ได้อีกแล้ว',
                  ['text' => 'ดูบัญชีทั้งหมด', 'href' => '#form']);
            flash_redirect('users.php');
        }
    ...
    <table>
        ...
        <tbody>
            <?php foreach ($users as $u): $isMe = (int) $u['id'] === $meId; ?>
                <tr id="row-<?= (int) $u['id'] ?>" class="<?= (int) $u['is_active'] === 1 ? '' : 'u-off' ?>">
                    <td class="wrap-cell">
                        <div class="u-name">
