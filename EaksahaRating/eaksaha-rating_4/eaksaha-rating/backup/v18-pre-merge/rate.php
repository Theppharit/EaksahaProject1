...
<body>
    ...
    <div class="rate-page">
        ...
        <div class="rate-card">
            ...
            <?php
            // ประกาศถึงลูกค้า — ขึ้นเฉพาะตอนผู้ดูแลติ๊ก "แสดงบนหน้าลูกค้า" ไว้เท่านั้น
            // ใช้ตอนระบบมีปัญหาแล้วอยากบอกลูกค้าตรงๆ ดีกว่าปล่อยให้เขาเดาเอง
            // อ่านจากไฟล์ ไม่แตะฐานข้อมูล จึงยังขึ้นได้แม้ฐานข้อมูลจะมีปัญหา
            require_once __DIR__ . '/admin/includes/notice.php';
            $cn = notice_get();
            if (!empty($cn['show_customer']) && notice_active($cn)):
            ?>
                <div class="cust-note <?= htmlspecialchars($cn['level']) ?>">
                    <?php if ($cn['title'] !== ''): ?><b><?= htmlspecialchars($cn['title']) ?></b><?php endif; ?>
                    <?php if ($cn['body']  !== ''): ?><span><?= nl2br(htmlspecialchars($cn['body'])) ?></span><?php endif; ?>
                </div>
            <?php endif; ?>
