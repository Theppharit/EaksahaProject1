...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        // ── ฝากข้อความถึงพนักงาน ──
        if (t.classList.contains('note-btn')) {
            var cn = cell(t);
            cn.querySelector('.score-view').hidden = true;
            cn.querySelector('.note-box').hidden = false;
            cn.querySelector('.note-text').focus();
            return;
        }
        if (t.classList.contains('note-cancel')) {
            var cn2 = cell(t);
            cn2.querySelector('.note-box').hidden = true;
            cn2.querySelector('.score-view').hidden = false;
            return;
        }
        if (t.classList.contains('note-save')) {
            var cn3 = cell(t);
            var box = cn3.querySelector('.note-text');
            var msg = box.value.trim();
            if (!msg) { box.focus(); return; }

            t.disabled = true;
            t.textContent = 'กำลังส่ง...';

            var nb = new URLSearchParams();
            nb.set('rating_id', cn3.dataset.rid);
            nb.set('note', msg);
            ...
            fetch('note_save.php', {
                ...
                .then(function (d) {
                    ...
                    var view = cn3.querySelector('.score-view');
                    ...
                    flag.textContent = d.count + ' ข้อความที่ฝากไว้แล้ว';
                    window.toast('success', 'ส่งถึง ' + d.to + ' แล้ว',
                                 'จะเห็นแจ้งเตือนในเมนู "ข้อความจากหัวหน้า" ตอนล็อกอินครั้งถัดไป');
                    box.value = '';
                    cn3.querySelector('.note-box').hidden = true;
                    view.hidden = false;
                    t.disabled = false;
                    t.textContent = 'ส่งถึงพนักงาน';
                })
                .catch(function (err) {
        ...
        if (t.classList.contains('score-edit-save')) {
            ...
            fetch('ai_override.php', {
                ...
                .then(function (d) {
                    ...
                    window.toast('success', 'แก้เป็น ' + n + ' ดาวแล้ว',
                                 'คะแนนใหม่มีผลกับแดชบอร์ดและรายงานทันที · ' + d.note);
                })
                .catch(function (err) {
                    window.toast('error', 'แก้ดาวไม่สำเร็จ', err.message);
                    t.disabled = false;
                    t.textContent = 'บันทึก';
                });
        }
