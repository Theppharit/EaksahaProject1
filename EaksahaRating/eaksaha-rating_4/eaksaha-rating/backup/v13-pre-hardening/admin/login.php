...
// หน้าแรกของแต่ละบทบาท — พนักงานขายไม่มีสิทธิ์เห็นแดชบอร์ดรวม
$landing = fn(string $role) => $role === 'sales' ? 'my_reviews.php' : 'dashboard.php';

// ถ้า login ค้างไว้อยู่แล้ว ให้ไปหน้าแรกของบทบาทนั้น
if (!empty($_SESSION['admin_id'])) {
    header('Location: ' . $landing($_SESSION['admin_role'] ?? 'admin'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ถ้ายังไม่ได้รัน roles_migration.sql คอลัมน์พวกนี้จะยังไม่มี
    // ให้ถือว่าเป็น admin ไปก่อน ระบบจะได้ไม่ล็อกคนออกทั้งหมด
    $role     = $admin['role']     ?? 'admin';
    $isActive = array_key_exists('is_active', (array) $admin) ? (int) $admin['is_active'] : 1;

    if ($admin && password_verify($password, $admin['password']) && $isActive === 1) {
        session_regenerate_id(true);      // กัน session fixation

        $_SESSION['admin_id']       = $admin['id'];
        $_SESSION['admin_username'] = $admin['username'];
        $_SESSION['admin_role']     = $role;
        $_SESSION['admin_staff_id'] = $admin['staff_id'] ?? null;
        $_SESSION['admin_display']  = ($admin['display_name'] ?? '') !== ''
            ? $admin['display_name']
            : $admin['username'];
    ...
    // บัญชีที่ถูกปิดใช้งาน บอกให้ตรงสาเหตุ จะได้ไม่งงว่ารหัสผิดหรือเปล่า
    $error = ($admin && password_verify($password, $admin['password']) && $isActive !== 1)
...
<body>
    ...
    <div class="page">
        ...
        <div class="login-card">
            <div class="lc-head">
                <h1>เข้าสู่ระบบ <span class="accent">ระบบ <?= htmlspecialchars(SYS_CODE) ?></span></h1>
                <p><?= htmlspecialchars(SYS_NAME) ?><br><?= htmlspecialchars(SYS_DESC) ?></p>
            </div>
