...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    ...
    flash('success', 'ลบแบรนด์แล้ว', 'พนักงานขายที่เคยสังกัดแบรนด์นี้จะกลายเป็น "ไม่ระบุแบรนด์"');
    flash_redirect('brands.php');
}
    ...
    if ($name === '' || $code === '') {
        ...
        try {
            if (($_POST['action']) === 'create') {
                ...
                flash('success', 'เพิ่มแบรนด์ ' . $name . ' แล้ว',
                      'โค้ด ' . $code . ' · สีนี้จะใช้บนหน้าประเมินของลูกค้าและในกราฟรายงาน',
                      ['text' => 'ดูในรายชื่อแบรนด์', 'href' => '#row-' . $newId]);
                flash_redirect('brands.php?hl=' . $newId);
            } else {
                $stmt = $pdo->prepare('UPDATE brands SET name = ?, code = ?, color = ?, sort_order = ? WHERE id = ?');
                $stmt->execute([$name, $code, $color, $sort, $id]);
                flash('success', 'บันทึกการแก้ไขแล้ว', $name . ' · โค้ด ' . $code,
                      ['text' => 'ดูในรายชื่อแบรนด์', 'href' => '#row-' . $id]);
                flash_redirect('brands.php?hl=' . $id);
            }
            $messageType = 'success';
...
<?php if ($message): ?>
    <script>window.toast('error', 'บันทึกไม่สำเร็จ', <?= json_encode($message, JSON_UNESCAPED_UNICODE) ?>);</script>
<?php endif; ?>
            ...
            <?php else: ?>
                <?php foreach ($brands as $b): ?>
                    <tr id="row-<?= (int) $b['id'] ?>">
                        ...
                        <td><span class="brand-tag" style="<?= brand_tag_style($b['color']) ?>"><?= htmlspecialchars($b['name']) ?></span></td>
